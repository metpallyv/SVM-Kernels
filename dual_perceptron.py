#implementation of dual perceptron 
    ...
    for iter in range(max_iterations):
        for i in range(X.shape[0]):
            ...
            for j in range(X.shape[0]):
                dot_val = np.dot(X[j], X[i])
                val= alpha[j] * Y[j] * dot_val#np.dot(X[i], X[j])
                sum = sum + val
            if sum <= 0:
                sum = -1
            elif sum > 0:
...
def test(train_X,train_Y,test_X,test_Y,alpha):
    m = test_Y.size
    right = 0
    for i in range(m):
        s = 0
...
def main(argv):
    ...
    for i in range(len(training_x)):
        ...
        print theta
        test(training_x[i],training_y[i],test_x[i],test_y[i],theta)
